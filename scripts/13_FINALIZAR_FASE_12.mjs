...
await updateJson(join(root, "package.json"), (pkg) => {
  pkg.version = version;
  pkg.description = "FinnacialUX Desktop — gestão financeira offline com SQLCipher, automações reversíveis, inteligência local e planejamento orientado por decisões.";
});
