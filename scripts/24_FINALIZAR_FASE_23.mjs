...
await updateJson(join(root, "package.json"), (pkg) => { pkg.version = version; pkg.description = "FinnacialUX Desktop — gestão financeira offline-first com backup externo criptografado, SQLCipher e Stronghold."; });
...
await updateJson(join(root, "src-tauri", "tauri.conf.json"), (tauri) => { tauri.version = version; if (tauri.bundle) { tauri.bundle.shortDescription = "Controle financeiro com redundância externa criptografada."; tauri.bundle.longDescription = "FinnacialUX Desktop 1.4 mantém dados em SQLCipher e espelha somente backups criptografados para outro volume ou pasta sincronizada."; } });
