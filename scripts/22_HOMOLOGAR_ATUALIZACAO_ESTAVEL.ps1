param(
  ...
  [switch]$Windows11,
  [switch]$CleanInstall,
  [switch]$UpgradeFrom110,
  [switch]$DataPreserved,
  [switch]$AutomaticBackupChecked,
  [switch]$DeduplicationChecked,
  [switch]$RetentionChecked,
  [switch]$StrongholdEncryptionChecked,
...
if ($LASTEXITCODE -ne 0) { throw "Os artefatos automáticos da versão 1.2.0 ainda não foram aprovados." }
$checks = [ordered]@{ windows10=[bool]$Windows10; windows11=[bool]$Windows11; cleanInstall=[bool]$CleanInstall; upgradeFrom110=[bool]$UpgradeFrom110; dataPreserved=[bool]$DataPreserved; automaticBackupChecked=[bool]$AutomaticBackupChecked; deduplicationChecked=[bool]$DeduplicationChecked; retentionChecked=[bool]$RetentionChecked; strongholdEncryptionChecked=[bool]$StrongholdEncryptionChecked; backupRestoreChecked=[bool]$BackupRestoreChecked; signatureChecked=[bool]$SignatureChecked; latestChannelChecked=[bool]$LatestChannelChecked }
$complete = -not (@($checks.Values) -contains $false)
$installer = Join-Path $ReleaseDir "FinnacialUX-Desktop_1.2.0_x64-setup.exe"
$report = [ordered]@{ formatVersion=3; product="FinnacialUX Desktop"; version=$Version; schemaVersion=14; promotedFrom="1.1.0"; releaseMode="stable-update"; previousReleaseEvidenceAvailable=$true; recordedAt=(Get-Date).ToUniversalTime().ToString("o"); installerSha256=(Get-FileHash $installer -Algorithm SHA256).Hash.ToLowerInvariant(); checks=$checks; manualMatrixComplete=$complete; latestChannelConfirmed=[bool]$LatestChannelChecked; status=if($complete){"approved-for-stable"}else{"manual-checks-pending"} }
...
Write-Host "HOMOLOGAÇÃO MANUAL DA ATUALIZAÇÃO 1.2.0 CONCLUÍDA" -ForegroundColor Green
Write-Host "Próximo passo: .\22_PUBLICAR_ATUALIZACAO_ESTAVEL.cmd"
