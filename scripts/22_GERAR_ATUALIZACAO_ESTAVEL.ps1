param(
  [switch]$SkipQuality,
  [string]$PreviousReleaseDirectory = "releases\1.1.0",
  [switch]$ForceRebuild
)
...
Set-Location $Root
$Version = "1.2.0"
$Notes = ".\release\RELEASE_NOTES_1_2_0.md"
...
if ($LASTEXITCODE -ne 0) { throw "A fonte da versão 1.2.0 não passou pela validação de release." }
node scripts\stable-release.mjs verify-promotion $Root $PreviousReleaseDirectory
if ($LASTEXITCODE -ne 0) { throw "A versão 1.1.0 não possui evidência de homologação suficiente." }
if (-not $SkipQuality) { & ".\22_VALIDAR_FASE_21.cmd"; if ($LASTEXITCODE -ne 0) { throw "A validação da Fase 21 falhou." } }
node scripts\stable-release.mjs prepare $Root $PreviousReleaseDirectory
if ($LASTEXITCODE -ne 0) { throw "Não foi possível preparar os manifestos da atualização 1.2.0." }
...
else { & ".\scripts\05_GERAR_RELEASE.ps1" -Version $Version -NotesFile $Notes; if ($LASTEXITCODE -ne 0) { throw "A geração assinada da atualização 1.2.0 falhou." } }
node scripts\stable-release.mjs prepare $Root $PreviousReleaseDirectory
if ($LASTEXITCODE -ne 0) { throw "Não foi possível sincronizar a documentação final da atualização 1.2.0." }
node scripts\stable-release.mjs verify-artifacts $Root "releases\$Version"
if ($LASTEXITCODE -ne 0) { throw "Os artefatos da atualização 1.2.0 são inválidos." }
Write-Host ""; Write-Host "ATUALIZAÇÃO ESTÁVEL 1.2.0 GERADA COM SUCESSO" -ForegroundColor Green; Write-Host "Pasta: releases\1.2.0"; Write-Host "Execute: .\22_HOMOLOGAR_ATUALIZACAO_ESTAVEL.cmd"
