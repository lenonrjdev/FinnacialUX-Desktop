...
try {
  ...
  Write-Host "FINNACIALUX DESKTOP - VALIDACAO DO HOTFIX 24.0.8"
  ...
  Write-Host "==> Confirmando a leitura robusta do EKU"
  Invoke-CheckedCommand -Command $Node -Arguments @(
    $PatchScript,
    "verify",
    $ProjectRoot
  ) -FailureMessage "O validador EKU ainda possui a regra incorreta."
  ...
  Write-Host ""
  Write-Host "HOTFIX 24.0.8 VALIDADO COM SUCESSO"
  Write-Host "Versao: 1.5.0"
  Write-Host "Schema: 14 (congelado)"
  Write-Host "EKU, certificado, SignTool e suite acumulada aprovados."
}
catch {
  Write-Host ""
