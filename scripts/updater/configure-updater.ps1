...
if (-not (Test-Path ".\node_modules")) {
  throw "Dependências não instaladas. Execute primeiro .\01_RODAR_PROJETO.cmd"
}
...
$KeyDirectory = Join-Path $UserProfile ".finnacialux-release"
...
if (-not (Test-Path $PrivateKeyPath)) {
  ...
  Invoke-FinnacialuxNativeCommand $Npm @("run", "tauri", "--", "signer", "generate", "-w", $PrivateKeyPath) "Não foi possível gerar as chaves do updater."
}
...
Write-Host "Chave privada: confirmada fora do projeto." -ForegroundColor Yellow
...
Write-Host "Faça duas cópias seguras da chave e da senha. Depois use .\03_VALIDAR_E_PREPARAR_ATUALIZACAO.cmd"
