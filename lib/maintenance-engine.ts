...
export function createMaintenanceReport(
  ...
  const checks: MaintenanceCheck[] = [
    check(
      "version",
      "Versão operacional",
      snapshot.currentVersion === "1.3.0"
        ? "A manutenção pós-lançamento está alinhada à versão 1.3.0."
        : `Versão esperada 1.3.0; encontrada ${snapshot.currentVersion}.`,
      snapshot.currentVersion === "1.3.0" ? "passed" : "blocked",
    ),
    check(
      "schema",
      "Schema congelado",
